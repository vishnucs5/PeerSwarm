...
class TestFlowIntegration:
    ...
    def test_quality_score_creation(self):
        ...
        score = QualityScore(
            ...
            logical_coherence=9, completeness=7, clarity=8,
            overall=7.8, iteration=1,
        )
        assert score.overall == 7.8
        assert score.improved_from_previous is False

    def test_quality_score_hard_gate(self):
        from src.models.quality import QualityScore, HardGateFailure, QualityDimension
        score = QualityScore(
            factual_accuracy=5, source_quality=8,
            logical_coherence=8, completeness=8, clarity=8,
            overall=7.4, iteration=1,
            hard_gate_failures=[
                HardGateFailure(
    ...
    def test_flow_state_routing_action(self):
        ...
        assert result in ("evaluate", "write", "write_with_warning",
                          "revise_research", "revise_analysis", "revise_writing")
...
class TestModelValidation:
    ...
    def test_score_improvement_tracking(self):
        from src.models.quality import QualityScore
        prev = QualityScore(
            factual_accuracy=5, source_quality=5,
            logical_coherence=5, completeness=5, clarity=5,
            overall=5.0, iteration=1,
        )
        curr = QualityScore(
            factual_accuracy=8, source_quality=8,
            logical_coherence=8, completeness=8, clarity=8,
            overall=8.0, iteration=2,
        )
...
class TestMetricsIntegration:
    ...
    def test_score_to_grade_roundtrip(self):
        from src.evaluation.metrics import score_to_grade, compute_weighted_score
        scores = {"factual_accuracy": 9, "source_quality": 9,
                   "logical_coherence": 9, "completeness": 9, "clarity": 9}
        weighted = compute_weighted_score(scores)
