...
and accessible.""",
    ...
    def execute(self, context: AgentContext, **kwargs) -> FinalReport:
        ...
        report = FinalReport(
            ...
            question=context.question,
            executive_summary=synthesis.executive_summary if hasattr(synthesis, 'executive_summary') else "",
            sections=sections,
    ...
    def _llm_write(self, question: str, synthesis: Any, context: AgentContext) -> str | None:
        ...
        if hasattr(synthesis, 'clusters'):
            for c in synthesis.clusters:
                ...
                clusters_text += f"\n### {c.theme}\n{findings_text}\n"
        insights = "\n".join(f"- {i}" for i in (synthesis.key_insights if hasattr(synthesis, 'key_insights') else []))
        limitations = "\n".join(f"- {l}" for l in (synthesis.limitations if hasattr(synthesis, 'limitations') else []))
    ...
    def _build_sections(self, synthesis: Any) -> list[dict[str, Any]]:
        ...
        sections.append({
            ...
            "content": synthesis.executive_summary if hasattr(synthesis, 'executive_summary') else "",
            "citations": [],
            "order": 0,
        })
    ...
    def _generate_markdown(self, question: str, synthesis: Any, sections: list[dict[str, Any]],
                          quality_score: Any) -> str:
