...
from src.ui.components.styling import divider, hero
...
def render():
    hero(
        ...
        "Submit a question to the multi-agent research system with automated quality loops.",
    )
    ...
    with col_form:
        ...
        with st.form("research_form", clear_on_submit=False):
            ...
            with col1:
                max_iter = st.slider(
                    "Max Iterations", 1, 5, 3, help="How many quality improvement cycles to run"
                )
                tags = st.text_input(
                    "Tags",
                    placeholder="rag, llm, survey",
            ...
            with col2:
                ...
                priority = st.selectbox(
                    "Priority", ["normal", "low", "high"], help="Job priority level"
                )
    ...
    with col_templates:
        st.markdown(
            '<div class="card" style="margin-bottom: var(--space-4)">', unsafe_allow_html=True
        )
        st.markdown(
        ...
        st.markdown('<div class="card">', unsafe_allow_html=True)
        st.markdown(
            '<div class="card-header"><span class="card-title">💡 Example Questions</span></div>',
            unsafe_allow_html=True,
        )
        st.markdown('<div class="card-body">', unsafe_allow_html=True)
