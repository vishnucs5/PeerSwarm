...
from src.ui.components.styling import divider, metric_card, status_dot
...
def _health_card(
    name: str, icon: str, status: str = "healthy", latency: str = "—", extra: str = ""
):
    status_class = status.lower().replace(" ", "-")
    dot = status_dot(
        "success" if status == "healthy" else "warning" if "degraded" in status.lower() else "error"
    )
    st.markdown(
        f"""
...
def render():
    ...
    if health:
        ...
        st.caption(
            f"Version: {health.get('version', '—')} &middot; Timestamp: {health.get('timestamp', '—')}"
        )

        st.markdown(
            """
        <div class="health-grid" style="margin-top: var(--space-6);">""",
            unsafe_allow_html=True,
        )
        ...
        st.markdown("</div>", unsafe_allow_html=True)
        st.error(
            "⚠️ API server is unreachable. Make sure the server is running and the API URL is correct."
        )
